...
// GET /api/ticker?symbols=ES=F,NQ=F,...  — Yahoo Finance v8/chart 서버 프록시
router.get('/ticker', async (req, res) => {
  const symbols = (req.query.symbols || '').split(',').map(s => s.trim()).filter(Boolean).slice(0, 30);
  ...
  const fetchOne = async (sym) => {
    try {
      ...
      return { symbol: sym, shortName: meta.shortName || sym, price, change };
    } catch(e) { return null; }
  };

  const results = await Promise.allSettled(symbols.map(fetchOne));
  const quotes = results.map(r => r.value).filter(Boolean);
